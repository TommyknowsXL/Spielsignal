...
const redaktionIndex = read("src/pages/redaktion/index.astro");
const redaktionPreview = read("src/pages/redaktion/vorschau/[slug].astro");
const redaktionEdit = read("src/pages/redaktion/bearbeiten/[slug].astro");
const redaktionWorkflow = read("src/pages/redaktion/workflow.astro");
...
assert.match(redaktionIndex, /formatDateTime\(draft\.data\.createdAt\)/);
assert.match(redaktionIndex, /formatDateTime\(candidate\.createdAt\)/);
assert.match(redaktionIndex, /Candidate ID kopieren/);
assert.match(redaktionIndex, /Create Editorial Draft/);
assert.match(redaktionIndex, /needs-source-review/);
assert.match(redaktionIndex, /Recherche-Stub/);
assert.match(redaktionIndex, /href=\{`\/redaktion\/vorschau\/\$\{draft\.data\.slug\}\/`\}>\{draft\.data\.title\}/);
assert.match(redaktionIndex, /Vorschau oeffnen/);
assert.match(redaktionIndex, /Bearbeiten/);
assert.match(redaktionIndex, /Workflow ansehen/);
assert.match(redaktionIndex, /Freigabe vorbereiten/);
assert.match(redaktionIndex, /Ablehnen vorbereiten/);
assert.match(redaktionIndex, /action=approve/);
assert.match(redaktionIndex, /action=reject/);
assert.match(redaktionIndex, /Aktionen erzeugen Patch-\/PR-Hinweise/);
...
assert.match(redaktionPreview, /AdSlot/);
assert.match(redaktionPreview, /Interne Pruefung/);
assert.doesNotMatch(redaktionPreview, /getCollection\("articles"/);
assert.match(redaktionEdit, /robots="noindex, nofollow"/);
assert.match(redaktionEdit, /schreibt nicht direkt aus dem Browser ins Repository/);
assert.match(redaktionEdit, /Pull Request/);
assert.match(redaktionEdit, /Freigeben veroeffentlicht nicht direkt auf main/);
assert.match(redaktionEdit, /Ablehnen erzeugt keinen Artikel/);
assert.match(redaktionWorkflow, /Deployment Protection/);
assert.match(redaktionWorkflow, /Diese Seite ist eine Anleitung/);
assert.match(redaktionWorkflow, /Hier werden keine Drafts gespeichert, freigegeben oder abgelehnt/);
assert.match(redaktionWorkflow, /Zurueck zur Redaktion/);
assert.match(redaktionWorkflow, /Eine unbekannte URL gilt nicht als Schutz/);
assert.match(astroConfig, /filter:\s*\(page\) => !page\.includes\("\/redaktion\/"\)/);
